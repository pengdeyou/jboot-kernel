...
package org.jboot.develop.support;
...
import org.jboot.develop.constant.DevelopConstant;
...
public class BladeCodeGenerator {
	 ...
	 * 代码生成的包名
	 */
	private String packageName = "org.jboot.test";
	/**
	 * 代码后端生成的地址
	 */
	 ...
	 * 需要生成的表名(两者只能取其一)
	 */
	private String[] includeTables = {"tb_test"};
	/**
	 * 需要排除的表名(两者只能取其一)
	 */
	...
	public void run() {
		...
		FastAutoGenerator.create(url, username, password)
			...
			.strategyConfig(builder -> builder.addTablePrefix(tablePrefix).addInclude(includeTables).addExclude(excludeTables)
				.entityBuilder().naming(NamingStrategy.underline_to_camel).columnNaming(NamingStrategy.underline_to_camel).enableLombok().superClass("org.jboot.kernel.mp.base.BaseEntity").addSuperEntityColumns(superEntityColumns).enableFileOverride()
				.serviceBuilder().superServiceClass("org.jboot.kernel.mp.base.BaseService").superServiceImplClass("org.jboot.kernel.mp.base.BaseServiceImpl").formatServiceFileName("I%sService").formatServiceImplFileName("%sServiceImpl").enableFileOverride()
				...
				.controllerBuilder().superClass("org.jboot.kernel.boot.ctrl.BladeController").formatFileName("%sController").enableRestStyle().enableHyphenStyle().enableFileOverride()
			)
